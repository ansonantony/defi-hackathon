...
const LiquidityHistory = ({ GET_ALL_LIQUIDITY }) => {
  ...
  <section>
    <div className="container">
      <div className="row mediumpadding120">
            ...
            <tbody>
              {
                liquidity?.map((liquidity,index)=>(
                  ...
                  className="crumina-module
                   crumina-pricing-table pricing-table--style-table-blurring c-brown"
                  >
                    <td>{index+1}</td>
                    <td >
                   ...
                   <div className="currency-details-item">
                         <div onClick={()=> navigator.clipboard.writeText(
                           ...
                           className={`value ${index %2===0 ? "c-green-success":"c-red-light"} c-primary`}
                           >
              ...
              <td>
                <a href={
                  liquidity?.network=="80001"
                  ?`https://amoy.polygonscan.com/tx/${liquidity?.transactionHash}` 
                  : liquidity?.network=="157"
                  ?`https://polygonscan.io/tx/${liquidity?.transactionHash}` 
                 : liquidity?.network=="1" 
                 ? `https://etherscan.com/tx/${liquidity?.transactionHash}` 
                 : liquidity?.network=="5" 
                 ? `https://goerli.etherscan.io/tx/${liquidity?.transactionHash}`  
                 :liquidity?.network == "11155111"  
                 ?`https://sepoli.etherscan.io/tx/${liquidity?.transactionHash}`  
                 :`https://amoy.polygonscan.com/tx/${liquidity?.transactionHash}` 
                }
                target="_blank"
